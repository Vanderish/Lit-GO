...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Check triggers set by LandingPage
    const sTab = sessionStorage.getItem('open_sandbox_tab');
    if (sTab) {
      sessionStorage.removeItem('open_sandbox_tab');
      setActiveTab(sTab);
    }

    const rModal = sessionStorage.getItem('open_radar_modal');
    if (rModal) {
      sessionStorage.removeItem('open_radar_modal');
      setRadarModalOpen(true);
    }

    const mId = sessionStorage.getItem('open_module_id');
    if (mId) {
      sessionStorage.removeItem('open_module_id');
  ...
  // Deepfake Magnifier Move Handler
  const handleMagMove = (e) => {
    if (!magBoxRef.current || !magImgRef.current || !magLensRef.current) return;
    const box = magBoxRef.current;
    const img = magImgRef.current;
  ...
  const handleFactClick = (id) => {
    setFactWords(prev => prev.map(w => {
      if (w.id === id && !w.clicked) {
        if (w.isHallu) {
          setFactScore(s => {
            const nextScore = Math.min(3, s + 1);
            if (nextScore === 3) showToast('Semua halusinasi berhasil teridentifikasi!', 'success');
            return nextScore;
          });
          setFactFeedback(`Tepat! Halusinasi/Bias: ${w.exp}`);
        } else {
          setFactFeedback('Informasi Valid: Kalimat ini berbasis data faktual yang dapat diverifikasi.');
        }
        return { ...w, clicked: true };
      }
      return w;
    }));
  };
  ...
  const resetHallucination = () => {
    ...
    setFactWords(prev => prev.map(w => ({ ...w, clicked: false })));
    showToast('Simulasi direset.', 'info');
  };

  // Ethical Dilemma Chooser
  ...
  const answerQuiz = (correct) => {
    if (correct) {
      ...
      const id = activeModId;
      const doneModules = state.doneModules.includes(id) ? state.doneModules : [...state.doneModules, id];
      const badges = [...state.badges];

      if (state.hasRadar && !badges.includes(1)) badges.push(1);
      if (doneModules.includes(2) && !badges.includes(2)) badges.push(2);
      if (doneModules.includes(3) && !badges.includes(3)) badges.push(3);
      if (doneModules.includes(4) && doneModules.includes(5) && !badges.includes(4)) badges.push(4);
      if (doneModules.length === 6 && state.hasRadar && !badges.includes(5)) badges.push(5);

      saveState({ ...state, doneModules, badges });
      setTimeout(() => {
  ...
  const handleResetData = () => {
    ...
    showToast('Seluruh data progres telah direset.', 'info');
  };

  // Computations for HUD
  const doneCount = state.doneModules ? state.doneModules.length : 0;
  const badgeCount = state.badges ? state.badges.length : 0;
  const pts = (doneCount * 250 + badgeCount * 100).toLocaleString();
  const lv = Math.max(1, Math.floor(doneCount / 2) + 1);
  const expPct = Math.round((doneCount / 6) * 100);

  // Compute Prompt Parser Stats
  const computePromptStats = (text) => {
    if (!text || !text.trim()) return { score: 0, safe: true, ctx: false, ins: false, fmt: false };
    ...
    return { score, safe, ctx, ins, fmt };
  };

  const promptStats = computePromptStats(promptText);
  ...
  const radarOptions = {
    ...
    scales: {
      r: {
        ...
        ticks: { display: false },
        pointLabels: { font: { size: 10, family: 'Plus Jakarta Sans', weight: 'bold' }, color: '#334155' }
      }
    },
  ...
  return (
    <div className="dashboard-root">
      ...
      <nav>
        <div className="nav-inner">
          ...
          <div className="nav-hud">
            ...
            <button className="btn-reset" onClick={() => setConfirmModalOpen(true)} title="Reset Progres Platform">
            ...
            <div className="hub-lobby-grid">
              ...
              <div className="hub-center">
                <div className="mascot-dialog">
                  <i className="fa-solid fa-quote-left" style={{ color: 'var(--indigo)' }}></i> Halo! Yuk mulai dari Radar Readiness untuk kenali level literasi AI kamu.
                </div>
                ...
                <div className="sandbox-tab-content active" id="tab-deepfake">
                  <div className="lab-inner">
                    ...
                    <div className="lab-sidebar">
                      ...
                      <button className="btn-lab" onClick={() => { if (!state.badges.includes(2)) { saveState({ ...state, badges: [...state.badges, 2] }); showToast('Inspeksi selesai! Badge diperbarui.', 'amber'); } else { showToast('Inspeksi selesai!', 'success'); } }}>
                  ...
                  <div className="lab-inner">
                    <div className="lab-box">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                        ...
                        <button className="btn-lab-ghost" style={{ fontSize: '0.74rem', padding: '4px 12px' }} onClick={() => { setPromptText('Bertindak sebagai tutor AI beretika. Jelaskan konteks definisi Machine Learning dalam format tabel ringkas untuk siswa SMA, tanpa meminta password atau data rahasia pengguna.'); showToast('Contoh prompt aman berhasil dimuat.', 'info'); }}>
          ...
          <div className="hub-section" id="hub-rewards">
            ...
            <div className="panel" style={{ padding: '24px', marginTop: '20px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '20px', justifyContent: 'space-between' }}>
              ...
              <div className="cert-form-row">
                ...
                <button className="btn-cert" id="cert-btn" onClick={() => { if (!certName.trim()) showToast('Masukkan nama lengkap!', 'error'); else showToast(`🎓 Selamat ${certName}! Sertifikat Lit-GO berhasil dibuat.`, 'success'); }} disabled={state.badges.length < 4}>
          ...
          <div className="footer-grid">
            ...
            <div className="footer-nav-col">
              ...
              <ul className="footer-links">
                ...
                <li><a href="#hub-radar">Radar Asesmen Mandiri</a></li>
                <li><a href="#hub-modules">Modul Learning Path</a></li>
                <li><a href="#hub-sandbox">Lab Interaktif AI</a></li>
                <li><a href="#hub-rewards">Sertifikat &amp; E-Badge</a></li>
              </ul>
            </div>
            ...
            <div className="footer-nav-col">
              ...
              <ul className="footer-links">
                <li><a href="javascript:void(0)">Ukuran Teks (16-20px)</a></li>
                <li><a href="javascript:void(0)">Font Ramah Disleksia</a></li>
                <li><a href="javascript:void(0)">Mode Kontras Tinggi</a></li>
                <li><a href="javascript:void(0)">Text-to-Speech (TTS)</a></li>
              </ul>
            </div>
      ...
      {/* MODALS */}
      {isRadarModalOpen && (
        <div className="modal-overlay" id="modal-radar" style={{ display: 'flex' }}>
              ...
              {[
                ...
                <div key={i}>
                  ...
                  <input type="range" min="1" max="5" value={radarAnswers[i]} onChange={(e) => {
                    ...
                    newAns[i] = parseInt(e.target.value, 10);
      ...
      {isModuleModalOpen && activeMod && (
        <div className="modal-overlay" id="modal-module" style={{ display: 'flex' }}>
          <div className="modal-box">
            <div className="modal-head">
              <div>
